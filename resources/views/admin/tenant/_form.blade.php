@php
  // Map <value> => <label> (HARUS cocok dengan rules Request)
...
<div class="row g-3">
  ...
  {{-- USER --}}
  <div class="col-md-6">
    <label for="user_id" class="form-label">Pengguna (akun)</label>
    <select id="user_id" name="user_id"
            class="form-select @error('user_id') is-invalid @enderror" required>
      <option value="">— Pilih User —</option>
      @foreach ($users as $u)
        <option value="{{ $u->id }}"
          @selected(old('user_id', $tenant->user_id ?? null) == $u->id)>
          {{ $u->name }} — {{ $u->email }}
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-6">
    ...
    <small class="text-muted">Hanya kamar “Tersedia” yang bisa dipilih. Kamar saat ini tetap dapat dipilih.</small>
  ...
  <div class="col-md-6">
    ...
    <input id="nama_penghuni" type="text" name="nama_penghuni"
           ...
           value="{{ old('nama_penghuni', $tenant->nama_penghuni ?? '') }}" required>
  ...
  <div class="col-md-3">
    ...
    <select id="jenis_kelamin" name="jenis_kelamin"
      ...
      @foreach ($genders as $val => $label)
        <option value="{{ $val }}"
          @selected(old('jenis_kelamin', $tenant->jenis_kelamin ?? '') === $val)>
          {{ $label }}
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-3">
    ...
    <select id="status" name="status"
      ...
      @foreach ($statuses as $val => $label)
        <option value="{{ $val }}"
          @selected(old('status', $tenant->status ?? 'active') === $val)>
          {{ $label }}
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-6">
    ...
    <input id="telpon" type="tel" name="telpon"
           ...
           value="{{ old('telpon', $tenant->telpon ?? '') }}"
           placeholder="08xxxxxxxxxx"
           pattern="[0-9 +()\-]{6,20}">
    <small class="text-muted">Boleh angka, spasi, tanda + ( ), dan -</small>
  ...
  <div class="col-md-6">
    ...
    <select id="pekerjaan" name="pekerjaan"
            class="form-select @error('pekerjaan') is-invalid @enderror" required>
      <option value="">— Pilih Pekerjaan —</option>
      @foreach ($jobs as $val => $label)
        <option value="{{ $val }}"
          @selected(old('pekerjaan', $tenant->pekerjaan ?? '') === $val)>
          {{ $label }}
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-6">
    ...
    <input id="nama_wali" type="text" name="nama_wali"
           ...
           value="{{ old('nama_wali', $tenant->nama_wali ?? '') }}">
  ...
  <div class="col-md-6">
    ...
    <input id="telpon_wali" type="tel" name="telpon_wali"
           class="form-control @error('telpon_wali') is-invalid @enderror"
           value="{{ old('telpon_wali', $tenant->telpon_wali ?? '') }}"
           pattern="[0-9 +()\-]{6,20}">
  ...
  <div class="col-md-6">
    ...
    <input id="tanggal_masuk" type="date" name="tanggal_masuk"
           ...
           value="{{ old('tanggal_masuk', isset($tenant->tanggal_masuk) ? \Illuminate\Support\Carbon::parse($tenant->tanggal_masuk)->format('Y-m-d') : '') }}"
           required>
  ...
  <div class="col-md-6">
    ...
    <input id="tanggal_keluar" type="date" name="tanggal_keluar"
           ...
           value="{{ old('tanggal_keluar', isset($tenant->tanggal_keluar) ? \Illuminate\Support\Carbon::parse($tenant->tanggal_keluar)->format('Y-m-d') : '') }}">
